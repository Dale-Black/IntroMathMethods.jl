...
# ╔═╡ f9a7e940-4d73-4346-82a4-3355b40787ab
# ╠═╡ show_logs = false
using CondaPkg; CondaPkg.add("SymPy")

# ╔═╡ b48d1dcf-13e4-4e5d-ad51-9293bdc63bfe
using PlutoUI, PythonCall, CairoMakie

# ╔═╡ 883ca046-1733-49b1-955c-fa7b2630b20e
sp = pyimport("sympy");

# ╔═╡ 9c2a9d8b-004d-4dec-89c0-d37ac0324f30
TableOfContents()

# ╔═╡ c3f02595-0313-48a3-b5a2-9ded064ac073
md"""
# Introduction
"""

# ╔═╡ a38c51dc-f2ba-4001-a6ee-eb6a4fc72502
md"""
# Differential Equations
"""

# ╔═╡ ebdf799e-d800-4825-a696-147f3f9c8e49
md"""
## Introduction
"""

# ╔═╡ 6cb58e54-fc42-42a8-a3cf-1e215ec87a7e
md"""
## ODEs with Constant Coefficients
"""

# ╔═╡ c5a0b9d8-ebbc-4a10-843e-59fc77589856
md"""
!!! info "Problem 10.2.3"
	```math
	\begin{align*}
	x(t) &= C \cos(\omega t) + D \sin(\omega t) \\
	x(t) &= E \cos(\omega t - \phi) \\
	\end{align*}
	```

	Find ``E`` and ``\phi`` in terms of ``C`` and ``D`` above
"""

# ╔═╡ 9b75032f-390c-4ad0-806d-125e4f2c6aad
md"""
!!! warning "By Hand"
	```math
	\begin{align*}
	...
	\end{align*}
	```
Given the expressions ``x(t) = C \cos(\omega t) + D \sin(\omega t)`` and ``x(t) = E \cos(\omega t - \phi)``, the amplitude ``E`` and phase ``\phi`` in terms of ``C`` and ``D`` are given by the formulas above.
"""

# ╔═╡ fade8bbd-e3b9-44e8-bb61-fdb58ecb2573
md"""
!!! info "Problem 10.2.5"

	Find the general solution of ``(D^3 - D^2 + D - 1)x = 0``. (Find one root by inspection and then proceed)
"""

# ╔═╡ 41307efc-f07e-4e8d-92f1-83c30ba47e0d
md"""
!!! warning "By Hand"
	```math
	\begin{align*}
	&D^3 - D^2 + D - 1 = 0 \\
	&(D - 1)(D^2 + D + 1) = 0 \\
	&D = 1, \quad D = -\frac{1}{2} + \frac{\sqrt{3}}{2}i, \quad D = -\frac{1}{2} - \frac{\sqrt{3}}{2}i \\
	\end{align*}
	```
	The roots of the polynomial are ``D = 1``, ``D = -1/2 + \sqrt{3}i/2``, and ``D = -1/2 - sqrt(3)i/2``. These roots correspond to solutions of the differential equation in the form ``x(t) = e^(rt)``. Therefore, the general solution of the differential equation is given by:

	```math
	x(t) = C_1 e^t + C_2 e^{-\frac{1}{2}t} \cos\left(\frac{\sqrt{3}}{2}t\right) + C_3 e^{-\frac{1}{2}t} \sin\left(\frac{\sqrt{3}}{2}t\right)
	```
where ``C_1``, ``C_2``, and ``C_3`` are arbitrary constants determined by initial conditions.
"""

# ╔═╡ 9159f6ba-3a00-44f8-9bd2-120f7e2f6bc4
md"""
!!! info "Problem 10.2.8"
	...
	```math
	\begin{align*}
	(D^2 + 42D + 1)x(t) = 0 \quad (\text{with } x(0) = 1, \dot{x}(0) = 0)
	\end{align*}
	```
	...
	```math
	\begin{align*}
	(D^3 + 3D^2 - 9D - 5)x(t) = 0 \quad (\text{5 is a root})
	\end{align*}
	```
...
# ╔═╡ c440418c-f055-4a04-8d6f-7bf9978b4259
md"""
!!! warning "By Hand"
	```math
	\begin{align*}
	&\text{(i) } D^2 + 42D + 1 = 0 \\
	&D = \frac{-42 \pm \sqrt{42^2 - 4(1)(1)}}{2} \\
	&D = \frac{-42 \pm \sqrt{1764 - 4}}{2} \\
	&D = \frac{-42 \pm 42\sqrt{1 - \frac{1}{441}}}{2} \\
	&\text{Given the initial conditions, the solution is:} \\
	&x(t) = e^{\frac{-42 + 42\sqrt{1 - \frac{1}{441}}}{2}t} + e^{\frac{-42 - 42\sqrt{1 - \frac{1}{441}}}{2}t} \\
	& \\
	&\text{(ii) } D^4 + 1 = 0 \\
	&\text{The roots are the fourth roots of -1, leading to the solutions:} \\
	&x(t) = e^{it/2}, \quad x(t) = e^{-it/2}, \quad x(t) = e^{3it/2}, \quad x(t) = e^{-3it/2} \\
	& \\
	&\text{(iii) } D^3 + 3D^2 - 9D - 5 = 0 \\
	&\text{Factoring gives:} \\
	&(D - 5)(D^2 + 8D + 1) = 0 \\
	&D = 5, \quad D = \frac{-8 \pm \sqrt{8^2 - 4(1)(1)}}{2} \\
	&D = 5, \quad D = \frac{-8 \pm \sqrt{60}}{2} \\
	& \\
	&\text{(iv) } (D + 1)^2(D^4 - 256) = 0 \\
	&\text{Factoring gives:} \\
	&(D + 1)^2(D^2 + 16)(D^2 - 16) = 0 \\
	&D = -1, \quad D = 4i, \quad D = -4i, \quad D = 4, \quad D = -4
	\end{align*}
	```
The roots of the characteristic equations correspond to solutions of the form ``e^{rt}``. These solutions form the general solutions of the given differential equations.

"""

# ╔═╡ 67f111ed-f9fa-4b6b-bd26-9383957ec172
md"""
# Summary
"""
