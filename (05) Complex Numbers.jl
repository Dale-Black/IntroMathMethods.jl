...
In the last few notebooks, we've explored the fascinating world of calculus, both in single and multiple dimensions, using the power of Julia and SymPy. We've delved into differentiation, integration, and even multivariate calculus. Now, we're ready to embark on another mathematical journey - this time into the realm of complex numbers.

Complex numbers, comprising both real and imaginary parts, are integral to many fields such as physics, engineering, and computer science. Understanding them thoroughly opens up a new dimension (quite literally!) in problem-solving. In this notebook, we'll study complex numbers in both Cartesian and Polar forms, and even explore some of their applications.

Key points we will cover include:

- Understanding the Cartesian and Polar forms of complex numbers
- Deriving and understanding key relationships and properties of complex numbers
- Applying complex number theory to solve problems

As with our previous notebooks, we will adopt an interactive approach, combining theoretical explanations with practical implementation using Julia's and SymPy. This will not only help solidify understanding of complex number theory but also give hands-on experience of applying these concepts programmatically.
"""
...
# ╔═╡ 6f9738c2-279d-4ff2-888c-e38c45154c14
md"""
...
``i`` behaves like real numbers in all manipulations involving addition and multiplication. The only new addition is that it's square is equal to -1. The general complex number then becomes
...
If the complex numbers ``z_1 = z_2``, then the parts of ``z_1`` and ``z_2`` must be exactly equal.
...
Complex conjugation (``z^*``) can be viewed as the process of replacing ``i`` by ``-i`` within the complex number

"""

# ╔═╡ 35a42177-1a33-4444-b34d-891ef17e8fd2
md"""
!!! info "Problem 5.2.5"

	Show that the polynomial with real coefficients has only real roots or complex roots that come in complex conjugate pairs
"""

# ╔═╡ 10907e21-5ffe-4ef7-9501-0f1c65debe9d
md"""
!!! warning "By Hand"
	  - Let's consider a polynomial ``p(x)`` with real coefficients and suppose that ``a + bi`` (where ``a`` and ``b`` are real numbers, and ``b ≠ 0``) is a root of this polynomial. This means that when we substitute ``x = a + bi`` into ``p(x)``, we get zero:
    ...
    ```
	  ...
	  - Now, take the complex conjugate of both sides of this equation. The left-hand side becomes the polynomial evaluated at the complex conjugate of the root, and the right-hand side remains zero (since the complex conjugate of zero is zero):
	...
	- Therefore, ``a - bi`` is also a root of the polynomial. This completes the proof.

	So, the roots of a polynomial with real coefficients will always be either real or pairs of complex conjugates.

"""

# ╔═╡ bd1cc0b5-f73a-4b0b-abf4-71daa9d566db
md"""
#### With SymPy
"""

# ╔═╡ ea2ba499-fa07-483e-a548-62008a19143c
x = sp.symbols("x")

# ╔═╡ 957aec8f-7db9-4965-81a7-ace65297c4e7
poly = sp.Poly(x^4 - x^2 + 1, domain = "RR")

# ╔═╡ 2ecc2b91-6cd8-4fad-8885-cdeda56bda94
roots = sp.solve(poly, x)

# ╔═╡ b8cfdc01-a6d0-4d81-99a5-469d4a4c5141
pyconvert(Vector, roots)[1:2]

# ╔═╡ 6f750740-4a3a-4f66-9f38-1168612c8f13
md"""
As we can see above, the roots do indeed come in complex pairs
"""

# ╔═╡ b050bcaf-ca25-496b-a312-9f6dd5385b6c
md"""
!!! info "Problem 5.2.6"
	...
	|Re \ z| &\leq |z| \\
	|Im \ z| &\leq |z| \\
	|z_1 + z_2|^2 &= |z_1|^2 + |z_2|^2 + 2 Re(z_1 z_2^*) \\
	|z_1 + z_2| &\leq |z_1| + |z_2| \\
	|z_1 z_2| &= |z_1||z_2| \\
	\end{align*}
	```
...
!!! warning "By Hand"
	...
	|z_1 + z_2| &≤ \sqrt{|z_1|^2 + |z_2|^2 + 2|z_1||z_2|} = |z_1| + |z_2|
	\end{align*}
	```
...
md"""
Euler gave us a remarkable identity
```math
\begin{align*}
...
Which can be proved taking the power series of ``e^{i\theta}`` with separate ``Re`` and ``Im`` parts

From this, we can gather the polar form of ``z``
```math
\begin{align*}
z &= x + iy \\
&= re^{i\theta} \\
...
!!! info "Problem 5.3.2"
	...
	For the following pairs of numbers, give their (i) polar form, (ii) complex conjugates, (iii) moduli, (iv) product, (v) quotient ``\frac{z_1}{z_2}``, and (5) complex conjugate of the quotient

	a) 
	```math
	\begin{align*}
	z_1 = \frac{1 + i}{\sqrt{2}} \ \ z_2 = \sqrt{3} - i
	\end{align*}
	```
	...
	```math
	\begin{align*}
	z_1 = \frac{3 + 4i}{3 - 4i} \ \ z_2 = \left[ \frac{1 + 2i}{1 - 3i} \right]^2
	\end{align*}
	```
"""
...
!!! warning "By Hand"
	...
	```math
	\begin{align*}
	z_1 z_2 &= \frac{\sqrt{3} - 1}{\sqrt{2}} + i \frac{\sqrt{3} + 1}{\sqrt{2}}
	\end{align*}
	```
...
# ╔═╡ 1369f03f-7fd5-48cc-b232-230f206f2f1e
r1_a, r2_a = sp.Abs(z1_a), sp.Abs(z2_a)

# ╔═╡ c1f5673a-87d2-4893-ae73-0a6de13c5009
θ1_a, θ2_a = sp.arg(z1_a), sp.arg(z2_a)

# ╔═╡ 8c7dd047-e3ca-4542-a5f2-556c356468bf
begin
	# i)
	polar_z1_a = r1_a * (sp.exp(sp.I * θ1_a))
	polar_z2_a = r2_a * (sp.exp(sp.I * θ2_a))
	polar_z1_a, polar_z2_a
end

# ╔═╡ 2d75d7d4-1b1e-42f2-bab9-828a4660552a
begin
	# ii) 
...
# ╔═╡ 45cd630e-6397-4fb0-bf1b-af37a32e34e8
begin
	# iii)
...
# ╔═╡ d5fb2a60-d640-4947-ad0a-6e54f4143438
# iv)
product_a = z1_a * z2_a
...
!!! warning "By hand"
	...
	- Simplify ``z_1`` and ``z_2``:
	
	```math
	\begin{align*}
	z_1 &= \frac{3 + 4i}{3 - 4i} = -1 + i \\
	z_2 &= \left[\frac{1 + 2i}{1 - 3i}\right]^2 = -1 - i
	\end{align*}
	```
	...
	vi) Complex conjugate of the quotient:
...
# ╔═╡ c4fbe958-3116-4396-b6f3-fbc17216870f
r1_b, r2_b = sp.Abs(z1_b), sp.Abs(z2_b)

# ╔═╡ 97bb556b-ff1c-4930-805d-e09e8f6e114c
θ1_b, θ2_b = sp.arg(z1_b), sp.arg(z2_b)

# ╔═╡ 9ab92681-eb8a-4801-9f2d-e3867d2b22b5
begin
	# i)
	polar_z1_b = r1_b * (sp.exp(sp.I * θ1_b))
	polar_z2_b = r2_b * (sp.exp(sp.I * θ2_b))
	polar_z1_b, polar_z2_b
end

# ╔═╡ 97e16d58-f973-4dcf-87bf-35227c8444cb
begin
	# ii) 
...
# ╔═╡ 5eec082f-302b-49d3-83c5-3ad64e4a1715
begin
	# iii)
...
# ╔═╡ d8036433-0cb0-4cc8-b3e6-e4d2452c2d98
# iv)
product_b = z1_b * z2_b
...
md"""
!!! info "Problem 5.3.4"
	Check the following trig identities by expressing them in terms of their complex exponentials
...
# ╔═╡ 47527b36-5122-4251-9c25-49d202395292
md"""
!!! warning "By Hand"
	...
	2 \sin(x) \cos(x) &= 2 \left(\frac{e^{ix} - e^{-ix}}{2i}\right) \left(\frac{e^{ix} + e^{-ix}}{2}\right) \\
	&= \frac{e^{i2x} - e^{-i2x}}{2i}
	\end{align*}
	```
...
# ╔═╡ d5916ea4-4b6c-4753-a46e-4dab2c4d0418
md"""
#### With SymPy
"""

# ╔═╡ 565c780c-9b9e-4036-8220-5a7ed246fe6b
sin_exp(x) = (sp.exp(sp.I*x) - sp.exp(-sp.I*x)) / (2*sp.I)

# ╔═╡ 1d1a914e-b5e1-4c86-86f3-441c4cc8bf74
sin_exp(x)

# ╔═╡ 1dd5fb8b-093f-429a-b3af-2733463b2327
cos_exp(x) = (sp.exp(sp.I*x) + sp.exp(-sp.I*x)) / 2

# ╔═╡ 506e8b58-7f65-47fa-bba7-6e7a6ba5cc8a
cos_exp(x)

# ╔═╡ 522c5fea-307f-4b1c-acc1-bdc16c240c6d
sp.simplify(sin_exp(x)^2 + cos_exp(x)^2) == 1

# ╔═╡ 95b7cd74-e310-44bc-93b3-5507d183b745
sp.simplify(sin_exp(2x)) == sp.simplify(2*sin_exp(x) * cos_exp(x))

# ╔═╡ 5f9e300a-6c0e-4cfb-ada0-e022acd9c316
 sp.simplify(cos_exp(2x)) == sp.simplify(cos_exp(x)^2 - sin_exp(x)^2)

# ╔═╡ 96d6213a-0ca9-4510-bd4a-4e59b3940b94
md"""
# Summary

Below are the highlights from this chapter
...
\end{align*}
```
- then ``x`` is the ``Re`` (real) part of ``z`` and ``y`` is the ``Im`` (imaginary) part

---

Polar form
- ``r`` is the modulus, ``\theta`` is the phase of argument ``z``
```math
\begin{align*}
z &= x + iy = re^{i\theta} \\
r &= \sqrt{x^2 + y^2} \\
...
- where we have Euler's identity
```math
\begin{align*}
e^{i \theta} = \cos(\theta) + i \sin(\theta)
\end{align*}
...
\sin(\theta) &= \frac{e^{i \theta} - ie^{-i \theta}}{2} \\
\end{align*}
```
...
- very useful relations and inequalities to rememeber
```math
\begin{align*}
|Re \ z| &\leq |z| \\
|Im \ z| &\leq |z| \\
